 ...
 */
const uint8_t MESH_DEFAULT_PSK[16] = {0xd4,0xf1,0xbb,0x3a,0x20,0x29,0x07,0x59,0xf0,0xbc,0xff,0xab,0xcf,0x4e,0x69,0x01};

// ─── Channel Configuration ─────────────────────────────────────────────────────
MeshChannel_t MeshChannel; //???

// ─── XOR Hash (Channels.cpp:27-33) ────────────────────────────────────────────

uint8_t MeshChannel_meshXorHash(const uint8_t *p, size_t len) 
{
  uint8_t code = 0;
 ...
 */
MeshCryptoKey_t MeshChannel_meshExpandPsk(const uint8_t *raw, uint8_t raw_len) 
{
  MeshCryptoKey_t k;
  memset(k.bytes, 0, sizeof(k.bytes));
  if (raw_len == 0) k.length = 0;   // no encryption
  else if (raw_len == 1) 
  {
    uint8_t idx = raw[0];
    if (idx == 0) k.length = 0;  // explicitly disabled
    else 
    {
 ...
 */
uint8_t MeshChannel_meshComputeChannelHash(const char *effective_name,const MeshCryptoKey_t *key) 
{
  uint8_t h = meshXorHash((const uint8_t *)effective_name, strlen(effective_name));
  if (key->length > 0) h ^= meshXorHash(key->bytes, key->length);
  return h;
}

// ─── Channel Table ─────────────────────────────────────────────────────────────

MeshChannelTable_t MeshChannelTable; //??? 
...
const char* MeshChannelTable_effectiveName(uint8_t idx)
{
  ...
  if (n[0] == '\0') return meshPresetName(preset);
  return n;
}
 ...
 */
int MeshChannelTable_addDefaultChannel() 
{
 ...
 */
int MeshChannelTable_findByHash(uint8_t wire_hash, MeshCryptoKey *out_key) const 
{
  for (uint8_t i = 0; i < count; i++) 
  {
 ...
 */
int MeshChannelTable_tryDecrypt(const uint8_t *payload, size_t payload_len,
               uint8_t wire_hash,
               uint32_t from_node, uint32_t packet_id,
               uint8_t *out_plaintext,
               MeshCryptoKey *out_key,
               bool (*validate_fn)(const uint8_t *, size_t) = NULL)
{
  for (uint8_t i = 0; i < count; i++) 
  {
    ...
    // Decrypt in-place
    if (!meshDecrypt(&channels[i].key, from_node, packet_id, out_plaintext, payload_len)) continue;
    // Validate if callback provided
    if (validate_fn && !validate_fn(out_plaintext, payload_len)) continue;
    if (out_key) *out_key = channels[i].key;
    return i;
  }
  return -1;
