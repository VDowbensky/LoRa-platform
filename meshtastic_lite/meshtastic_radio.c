...
MeshFreqConfig_t MeshFreqConfig;
 ...
 */
MeshRadioConfig_t MeshRadioConfig; 
 ...
 */
bool processRx(const uint8_t *raw, size_t raw_len, float rssi, float snr, MeshRxResult_t *result)
{
  ...
  {
    size_t pt_len = meshTryPkiDecrypt(&result->packet, &pki,&node_keys, plaintext);
    if (pt_len > 0) 
    {
                ...
                uint8_t tx_hop_limit = MESH_HOP_RELIABLE)
{
  ...
  size_t data_len = meshEncodeData(data_buf, sizeof(data_buf),portnum, payload, payload_len,false, ok_to_mqtt);
  ...
  return meshBuildPacket(out,to, node_num, pkt_id,
                          hop_limit, hop_start,
                  ...
                  uint8_t tx_hop_limit = MESH_HOP_RELIABLE)
{
  ...
  uint8_t data_buf[240];
  size_t data_len = meshEncodeData(data_buf, sizeof(data_buf),portnum, payload, payload_len);
  if (data_len == 0) return 0;
...
size_t buildTextDm(uint32_t to_node, const char *text, bool want_ack,
                    ...
                    uint8_t tx_hop_limit = MESH_HOP_RELIABLE)
{
  return buildDmTx(to_node, PORT_TEXT_MESSAGE,(const uint8_t *)text, strlen(text),want_ack, out, rand_fn, tx_hop_limit);
}
