...
meshy_node_t* find_or_create_node(uint32_t node_num) 
{
  // Find existing
...
void update_node_from_packet(const MeshRxResult *result) 
{
  ...
  else if (result->data.portnum == PORT_TELEMETRY) 
  {
    ...
    {
      if (tel.has_device_metrics) node->battery_level = tel.device_metrics.battery_level;
    }
  }
...
void build_msg(const MeshRxResult *result, meshy_msg_t *msg) 
{
...
//Configure RX_DONE inyerrupt, clear flags
  return true;
}
...
static void meshy_rx_task(void *arg) 
{
  ...
  while (s_running) 
  {
    ...
    if (s_xl9535->pin_read(XL9535_SX1262_DIO1) == 1) 
    {
      ...
      MeshRxResult_t result;
      if (s_session.processRx(raw, len, rssi, snr, &result)) 
      {
        //Increment decoded RX packet counter
        s_stats.rx_decoded++;
        // Update node table
        ...
        {
          int idx = (s_msg_count < MESHY_MSG_HISTORY) ? i : (s_msg_write_idx + i) % MESHY_MSG_HISTORY;
...
void meshy_tx_task(void *arg) 
{
  ...
  while (s_running) 
  {
    ...
    if (xQueueReceive(s_tx_queue, &req, pdMS_TO_TICKS(200)) == pdTRUE) 
    {
      ...
      s_sx1262->start_lora_transmit(Cpp_Bus_Driver::Sx126x::Chip_Mode::TX, 0,Cpp_Bus_Driver::Sx126x::Fallback_Mode::FS);
      ...
      if (s_sx1262->send_data(frame, frame_len)) 
      {
        ...
        {
          s_stats.tx_packets++;
          ESP_LOGI(TAG, "TX success: %zu bytes to %s", frame_len, req.to == MESH_ADDR_BROADCAST ? "broadcast" : "DM");
        } 
        else ESP_LOGE(TAG, "TX timeout");
      } 
      else 
      {
