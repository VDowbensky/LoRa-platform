...
- **Original**: Used `std::collections::HashMap` for skipped keys and session storage
- **Translation**: Implemented fixed-size array-based hash tables with linear probing
  - `SkippedKeysMap`: Fixed array of `MAX_SKIPPED_KEYS` entries with linear search
  - `SessionManager`: Fixed array of `MAX_PERSISTED_SESSIONS` entries
- **Reason**: C doesn't have a standard library HashMap. For embedded systems, fixed-size arrays are more appropriate and predictable for memory usage.

### 2. HeaplessVec Implementation
- **Original**: Used `heapless::Vec<u8, 256>` for bounded dynamic arrays
- **Translation**: Implemented custom struct with fixed-size array and length field
  - Structure contains `uint8_t data[256]` and `size_t len`
  - Implemented `heapless_vec_init()` and `heapless_vec_extend_from_slice()` functions
- **Reason**: C requires manual implementation of dynamic array behavior with bounds checking

### 3. Result and Option Types
- **Original**: Used `Result<T, E>` and `Option<T>` for error handling
- **Translation**: 
  - `Result`: Functions return `SessionError` enum directly, with output parameters for success values
  - `Option`: Functions return `bool` for success/failure, or `NULL` pointers
- **Example**: `encrypt()` returns `SessionError` with output parameters `header` and `ciphertext`

### 4. Static Mutable Variables
- **Original**: `static mut COUNTER: u64 = 0` with unsafe access
- **Translation**: `static uint64_t COUNTER = 0` (no special unsafe block needed)
- **Reason**: C has simpler static variable semantics, though thread safety is still a concern

### 5. Conditional Compilation
- **Original**: `#[cfg(target_arch = "xtensa")]`
- **Translation**: `#ifdef __XTENSA__`
- **Reason**: C uses preprocessor directives for conditional compilation

### 6. Method Implementations
- **Original**: `impl Session { fn method(&self) { ... } }`
- **Translation**: Free functions with explicit pointer parameter: `void session_method(Session* session)`
- **Reason**: C doesn't have methods; all functions are free functions with explicit `this` pointer

### 7. Clone Trait
- **Original**: `#[derive(Clone)]` for automatic cloning
- **Translation**: Manual memory copy operations using `memcpy()`
- **Note**: C requires explicit implementation of copy operations

### 8. Constant Time Comparison
- **Original**: Used `core::ptr::read_volatile()` to prevent optimization
- **Translation**: Declared result as `volatile uint8_t` to achieve same effect
- **Added**: `__attribute__((noinline))` to prevent function inlining

### 9. Random Number Generation
- **Xtensa**: Direct memory-mapped register access using volatile pointer
- **Non-Xtensa**: Pseudo-random generation using SHA256 hash of counter and stack address
- **Reason**: Preserved exact functionality from original implementation

### 10. String Handling
- **Original**: Rust string slices with automatic length tracking
- **Translation**: C string literals with explicit `strlen()` calls for HKDF operations
- **Reason**: C requires explicit length calculation for binary-safe operations

## Type System Differences

### 1. Fixed-Size Arrays
- **Original**: `[u8; 32]` array types
- **Translation**: `uint8_t array[32]` with explicit size
- **Arrays as parameters**: Pass by pointer in C vs. by value in Rust

### 2. Struct Field Visibility
- **Original**: Fields public by default in struct definitions
- **Translation**: All struct fields are public in C (no privacy control)
- **Note**: Encapsulation must be enforced through documentation

### 3. Integer Types
- **Original**: `u8`, `u32`, `u64`, `usize`
- **Translation**: `uint8_t`, `uint32_t`, `uint64_t`, `size_t`
- **Reason**: Used C99 standard integer types for portability

## Standard Library Equivalents

### 1. Memory Operations
- `copy_from_slice()` → `memcpy()`
- `chunks_mut()` → Manual loop with pointer arithmetic
- `iter().zip()` → Parallel index loops

### 2. Byte Order Operations
- `.to_le_bytes()` → `memcpy()` (assumes little-endian host)
- `::from_le_bytes()` → `memcpy()` (assumes little-endian host)
- **Note**: This assumes the target platform is little-endian. For big-endian systems, byte swapping would be needed.

### 3. Min/Max Operations
- `core::cmp::min()` → Inline ternary operator: `(a < b) ? a : b`

## Error Handling Approaches

### 1. Error Propagation
- **Original**: `?` operator for error propagation
- **Translation**: Explicit error checking with early returns
- **Pattern**: 
  ```c
  SessionError err = function();
  if (err != SESSION_ERROR_NONE) {
      return err;
  }
  ```

### 2. Result Values
- **Original**: `Ok((header, ciphertext))` returns tuple
- **Translation**: Output parameters passed by pointer
- **Example**: `session_encrypt(session, plaintext, pt_len, &header, &ciphertext)`

## Platform-Specific Implementations

### ESP-IDF NVS Functions
- **Xtensa implementation**: Full NVS (Non-Volatile Storage) save/load using ESP-IDF APIs
- **Non-xtensa stub**: Returns success without doing anything
- **Key functions**: `nvs_open()`, `nvs_set_u32()`, `nvs_set_blob()`, `nvs_get_blob()`, `nvs_commit()`, `nvs_close()`

### Key Name Generation
- Implemented manual string construction for NVS keys: `"sess_0"`, `"sess_1"`, etc.
- Limited to 2-digit indices (0-99) matching `MAX_PERSISTED_SESSIONS`

## Cryptographic Operations

### HKDF Usage
- **Original**: `Hkdf::derive()` method
- **Translation**: `hkdf_derive()` function with explicit length parameters
- **Note**: Assumes external implementation provides this function

### AES-256-CTR Mode
- Implemented manual CTR mode encryption/decryption
- Uses nonce construction with message counter
- Block counter for sequential keystream generation

### HMAC Implementation
- Manual HMAC-SHA256 construction using inner and outer hash
- Used for message authentication tags (16-byte truncated)
...
- **Reason**: Embedded systems typically avoid dynamic allocation

### 2. Stack Usage
- Large structures passed by pointer to avoid stack overflow
- `HeaplessVec` capacity limited to 256 bytes
- Session serialization uses fixed 225-byte buffer

### 3. Data Lifetime
- All session data has explicit ownership through `SessionManager`
- No automatic lifetime management - caller must manage struct lifetimes

## Potential Issues and Limitations

### 1. Endianness Assumption
- Code assumes little-endian architecture for byte order conversions
- Big-endian systems would need explicit byte swapping

### 2. Thread Safety
- Static `COUNTER` variable not thread-safe
- `SessionManager` not thread-safe
- **Mitigation**: Add mutex protection if multi-threaded access is needed

### 3. Hash Table Collisions
- Linear search for hash table operations
- Performance degrades with high collision rates
- **Mitigation**: Consider implementing proper hashing if performance is critical

### 4. Fixed Array Limits
- `MAX_SKIPPED_KEYS = 100`: May fail if more keys need to be skipped
- `MAX_PERSISTED_SESSIONS = 32`: Limited number of concurrent sessions
- `HEAPLESS_VEC_CAPACITY = 256`: Maximum message size limitation

### 5. Crypto Function Dependencies
- Assumes external implementation of: `x25519()`, `x25519_base()`, `hkdf_derive()`, `sha256_hash()`, `aes256_*()` functions
- These must be provided by linking with appropriate crypto library

### 6. Error Handling Granularity
- Some error conditions merged into single error codes
- Less detailed error information than Rust version

### 7. No Automatic Memory Cleanup
- Sensitive key material must be explicitly zeroed
- Consider adding explicit `session_destroy()` function to zero memory

### 8. NVS Key Naming
- Limited to 2-digit session indices (0-99)
- Could be extended for more sessions with longer key names

## Testing Recommendations

1. **Endianness Testing**: Verify on both little-endian and big-endian platforms
2. **Boundary Conditions**: Test with maximum message sizes and session counts
3. **Error Paths**: Verify all error codes are handled correctly
4. **Memory Safety**: Run with valgrind or similar tools to check for leaks
5. **Crypto Verification**: Compare outputs with reference implementations
6. **NVS Persistence**: Test save/load cycles on actual ESP32 hardware
7. **Thread Safety**: If using in multi-threaded context, add appropriate locking

## Integration Notes

To use this code:
1. Implement or link the required crypto functions (x25519, HKDF, AES, SHA256)
2. For ESP32, ensure ESP-IDF is properly configured
3. Add appropriate error checking in calling code
4. Consider adding memory zeroing for security-sensitive cleanup
5. Add mutex protection if used in multi-threaded context
 */
...
// ESP-IDF includes for xtensa architecture
#ifdef __XTENSA__
#include <nvs_flash.h>
#include <nvs.h>
#endif
...
static bool heapless_vec_extend_from_slice(HeaplessVec* vec, const uint8_t* data, size_t len) {
    if (vec->len + len > HEAPLESS_VEC_CAPACITY) {
        return false;
    }
...
static bool skipped_keys_map_insert(SkippedKeysMap* map, const uint8_t key_prefix[8], 
                                     uint64_t message_num, const uint8_t message_key[32]) {
...
static bool skipped_keys_map_remove(SkippedKeysMap* map, const uint8_t key_prefix[8], 
                                     uint64_t message_num, uint8_t message_key[32]) {
    for (size_t i = 0; i < MAX_SKIPPED_KEYS; i++) {
        if (map->entries[i].occupied &&
            memcmp(map->entries[i].key_prefix, key_prefix, 8) == 0 &&
            map->entries[i].message_num == message_num) {
...
static void compute_tag(const uint8_t key[32], const uint8_t* header, size_t header_len,
...
static void advance_send_ratchet(Session* session) {
    ...
    hkdf_derive(kdf_input, 64, (const uint8_t*)"root", 4, ROOT_KEY_INFO, 
                strlen((const char*)ROOT_KEY_INFO), session->root_key, 32);
    hkdf_derive(kdf_input, 64, (const uint8_t*)"chain", 5, CHAIN_KEY_INFO, 
                strlen((const char*)CHAIN_KEY_INFO), session->send_chain_key, 32);
...
static SessionError advance_recv_ratchet(Session* session, const uint8_t their_new_public[32]) {
    ...
    hkdf_derive(kdf_input, 64, (const uint8_t*)"root", 4, ROOT_KEY_INFO, 
                strlen((const char*)ROOT_KEY_INFO), session->root_key, 32);
    hkdf_derive(kdf_input, 64, (const uint8_t*)"chain", 5, CHAIN_KEY_INFO, 
                strlen((const char*)CHAIN_KEY_INFO), session->recv_chain_key, 32);
...
static SessionError skip_message_keys(Session* session, uint64_t until) {
    ...
    if (to_skip > MAX_SKIPPED_KEYS) {
        return SESSION_ERROR_TOO_MANY_SKIPPED;
    }
    ...
    while (session->recv_count < until) {
        ...
        hkdf_derive(session->recv_chain_key, 32, count_bytes, 8, MESSAGE_KEY_INFO, 
                    strlen((const char*)MESSAGE_KEY_INFO), message_key, 32);
        ...
        hkdf_derive(session->recv_chain_key, 32, (const uint8_t*)"chain-advance", 13, 
                    CHAIN_KEY_INFO, strlen((const char*)CHAIN_KEY_INFO), new_chain, 32);
...
static SessionError get_message_key(Session* session, const MessageHeader* header, uint8_t message_key[32]) {
    ...
    if (skipped_keys_map_remove(&session->skipped_keys, key_prefix, header->message_num, message_key)) {
        return SESSION_ERROR_NONE;
    }
...
static void session_new(Session* session, const SessionParams* params) {
    ...
    // Derive initial keys
    const uint8_t* salt = params->is_initiator ? 
        (const uint8_t*)"initiator-salt-v1" : (const uint8_t*)"responder-salt-v1";
    size_t salt_len = strlen((const char*)salt);
    
    hkdf_derive(params->shared_secret, 32, salt, salt_len, ROOT_KEY_INFO, 
                strlen((const char*)ROOT_KEY_INFO), session->root_key, 32);
    ...
    if (params->is_initiator) {
        hkdf_derive(kdf_input, 64, (const uint8_t*)"send", 4, CHAIN_KEY_INFO, 
                    strlen((const char*)CHAIN_KEY_INFO), session->send_chain_key, 32);
        hkdf_derive(kdf_input, 64, (const uint8_t*)"recv", 4, CHAIN_KEY_INFO, 
                    strlen((const char*)CHAIN_KEY_INFO), session->recv_chain_key, 32);
    } else {
        hkdf_derive(kdf_input, 64, (const uint8_t*)"recv", 4, CHAIN_KEY_INFO, 
                    strlen((const char*)CHAIN_KEY_INFO), session->send_chain_key, 32);
...
// Encrypt a message
static SessionError session_encrypt(Session* session, const uint8_t* plaintext, size_t pt_len,
                                    MessageHeader* header, HeaplessVec* ciphertext) {
    if (!session->established) {
        return SESSION_ERROR_NOT_ESTABLISHED;
    }
    ...
    hkdf_derive(session->send_chain_key, 32, count_bytes, 8, MESSAGE_KEY_INFO, 
                strlen((const char*)MESSAGE_KEY_INFO), message_key, 32);
    
    // Advance chain key
    uint8_t new_chain_key[32];
    hkdf_derive(session->send_chain_key, 32, (const uint8_t*)"chain-advance", 13, 
                CHAIN_KEY_INFO, strlen((const char*)CHAIN_KEY_INFO), new_chain_key, 32);
    ...
    if (!heapless_vec_extend_from_slice(ciphertext, plaintext, pt_len)) {
        return SESSION_ERROR_INVALID_FORMAT;
    }
    ...
    for (size_t i = 0; i < ciphertext->len; i += 16) {
        ...
        // XOR with plaintext
        for (size_t j = 0; j < chunk_size; j++) {
            ciphertext->data[i + j] ^= keystream[j];
        }
    ...
    if (!heapless_vec_extend_from_slice(ciphertext, tag, 16)) {
        return SESSION_ERROR_INVALID_FORMAT;
    }
    ...
    if (should_ratchet(session)) {
        advance_send_ratchet(session);
    }
...
// Decrypt a message
static SessionError session_decrypt(Session* session, const MessageHeader* header,
                                    const uint8_t* ciphertext, size_t ct_len,
                                    HeaplessVec* plaintext) {
    if (!session->established) {
        return SESSION_ERROR_NOT_ESTABLISHED;
    }
    ...
    if (!constant_time_eq(received_tag, 16, expected_tag, 16)) {
        return SESSION_ERROR_DECRYPTION_FAILED;
    }
    
    // Decrypt ciphertext
    heapless_vec_init(plaintext);
    if (!heapless_vec_extend_from_slice(plaintext, ct_without_tag, tag_start)) {
        return SESSION_ERROR_INVALID_FORMAT;
    }
    ...
    for (size_t i = 0; i < plaintext->len; i += 16) {
        ...
        // XOR with ciphertext
        for (size_t j = 0; j < chunk_size; j++) {
            plaintext->data[i + j] ^= keystream[j];
        }
...
static void session_derive_session_hint(const Session* session, uint64_t epoch, uint8_t hint[4]) {
    ...
    hkdf_derive(input, 40, (const uint8_t*)"hint", 4, SESSION_HINT_INFO, 
                strlen((const char*)SESSION_HINT_INFO), hint, 4);
}
...
static bool session_deserialize(const uint8_t* data, size_t len, Session* session) {
    if (len < SESSION_SERIALIZED_SIZE) {
        return false;
    }
...
static Session* session_manager_get_session(SessionManager* manager, const uint8_t peer_public[32]) {
    ...
    for (size_t i = 0; i < MAX_PERSISTED_SESSIONS; i++) {
        if (manager->entries[i].occupied && 
            memcmp(manager->entries[i].peer_key, key, 8) == 0) {
            return &manager->entries[i].session;
        }
    }
    ...
    for (size_t i = 0; i < MAX_PERSISTED_SESSIONS; i++) {
        if (manager->entries[i].occupied && 
            ...
            if (i == manager->count - 1) {
                ...
                while (manager->count > 0 && !manager->entries[manager->count - 1].occupied) {
                    manager->count--;
                }
            }
            return;
