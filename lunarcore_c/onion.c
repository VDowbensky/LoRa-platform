...
- **Rust**: Uses `heapless::Vec` with type parameters and const generics
- **C**: Implemented as separate struct types for each capacity/element type combination:
  - `HeaplessVec_u8_256`: For byte vectors with capacity 256
  - `HeaplessVec_u8_320`: For byte vectors with capacity 320
  - `HeaplessVec_RouteHop_MAX_HOPS`: For RouteHop vectors with capacity 7
  - `HeaplessVec_RouteHop_32`: For RouteHop vectors with capacity 32
- Each struct contains: `data` array, `len` (current size), and `capacity` (max size)
- All vector operations implemented as inline functions

### 2. Error Handling
- **Rust**: Uses `Result<T, E>` and `Option<T>` types
- **C**: Uses boolean return values (true for success, false for failure) with optional output parameters
- OnionError enum provides error codes via pointer parameter where needed

### 3. Method to Function Conversion
- All Rust methods converted to C functions with explicit `self` parameter as first argument
- Naming convention: `struct_name_method_name` (e.g., `onion_router_wrap`)
- Used `static inline` for simple getters and small utility functions

### 4. Memory Management
- All structures use stack allocation (no dynamic memory)
- Fixed-size arrays replace heap-allocated vectors
- Explicit `memcpy` for all data copying operations
- All functions return status or use output parameters

### 5. Constant Time Equality
- Preserved the security-critical constant-time comparison function
- Uses volatile read to prevent compiler optimizations that could leak timing information

### 6. Cryptographic Operations
- Declared external functions for crypto primitives (x25519, HKDF, AES256, SHA256)
- These must be provided by linking against appropriate crypto library
- Function signatures adapted to C conventions

### 7. Iterator Handling
- **Rust**: Uses iterator methods like `.iter()`, `.enumerate()`, `.rev()`
- **C**: Implemented using explicit for loops with array indexing
- Reverse iteration handled with `for (int i = len - 1; i >= 0; i--)`

### 8. HMAC Implementation
- Implemented HMAC-SHA256 directly in `compute_tag` function
- Uses standard HMAC construction: `H((K XOR opad) || H((K XOR ipad) || data))`
- Truncated to 16 bytes for authentication tag

### 9. AES-CTR Mode Implementation
- Implemented counter mode encryption in `encrypt_layer`
- Counter initialized to zero, incremented for each block
- XOR operation with keystream for encryption/decryption
- Same function used for both encrypt and decrypt (CTR mode property)

### 10. Type Conversions
- Explicit casts for integer type conversions
- Byte order handling for multi-byte values (hint encoding)
- Little-endian format used for epoch and block counter values

## Key Implementation Details

### Inline Functions
Most small functions marked `static inline` for performance, including:
- HeaplessVec operations
- Simple getters
- Utility functions
- Struct constructors

### Constants
- All constant values preserved exactly
- String constants declared as static byte arrays with explicit lengths

### Dependencies
External functions required:
- `x25519()`: ECDH key exchange
- `x25519_base()`: Public key derivation
- `hkdf_derive()`: HKDF key derivation
- `sha256_hash()`: SHA-256 hashing
- `aes256_new()`, `aes256_encrypt_block()`, `aes256_free()`: AES-256 operations

### Safety Considerations
1. All array bounds checked before access
2. Buffer overflow protection via capacity checks
3. Constant-time comparison for authentication tags
4. No pointer arithmetic beyond safe array indexing
5. All memory operations use explicit sizes

### Potential Issues
1. Requires external crypto library implementation
2. No thread safety (add mutexes if needed)
3. Stack usage may be high due to fixed-size buffers (consider for embedded systems)
4. No runtime capacity validation beyond compile-time constants

### Testing Recommendations
1. Verify crypto function implementations match expected behavior
2. Test with maximum capacity vectors
3. Verify constant-time equality doesn't optimize away
4. Test error paths and edge cases
5. Validate interoperability with original Rust implementation
 */
...
static inline bool onion_route_new(OnionRoute *route, const RouteHop *hops, size_t hops_len) {
    if (hops_len < MIN_HOPS || hops_len > MAX_HOPS) {
        return false;
    }

    heapless_vec_routehop_init(&route->hops);
    for (size_t i = 0; i < hops_len; i++) {
        if (!heapless_vec_routehop_push(&route->hops, &hops[i])) {
            return false;
        }
    }
...
static inline bool onion_router_wrap(OnionRouter *self, const uint8_t *payload, size_t payload_len,
    ...
    for (int i = (int)route->hops.len - 1; i >= 0; i--) {
        ...
        // Determine next hint
        uint16_t next_hint;
        if (i == (int)route->hops.len - 1) {
            next_hint = 0;
        } else {
            next_hint = route->hops.data[i + 1].hint;
        }
...
static bool onion_router_encrypt_layer(OnionRouter *self, const uint8_t key[32],
    ...
    if (!heapless_vec_u8_256_extend_from_slice(output, data, data_len)) {
        return false;
    }
    ...
    for (size_t offset = 0; offset < output->len; offset += 16) {
        ...
        for (size_t i = 0; i < chunk_size; i++) {
            output->data[offset + i] ^= keystream[i];
        }
...
static inline bool route_builder_build_route(const RouteBuilder *builder, const RouteHop *destination,
    ...
    if (builder->relays.len < relay_count) {
        return false;
    }
    ...
    for (size_t i = 0; i < relay_count; i++) {
        ...
        if (!heapless_vec_routehop_push(&hops, &builder->relays.data[relay_index])) {
            return false;
        }
    }

    // Add destination hop
    if (!heapless_vec_routehop_push(&hops, destination)) {
        return false;
    }
