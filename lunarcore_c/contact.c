...
### 1. **Heapless Vector Implementation**
   - **Rust**: Uses `heapless::Vec` with compile-time capacity bounds
   - **C**: Implemented custom structs (`HeaplessVec128`, `HeaplessVec64`, `HeaplessVec256`, `HeaplessVec512`) with fixed-size arrays and length tracking
   - Each vector type has helper functions for initialization, extending from slices, pushing bytes, and clearing

### 2. **Error Handling**
   - **Rust**: Uses `Result<T, E>` enums for error handling
   - **C**: Converted to return error codes directly or use boolean returns with optional out parameters
   - `Result<Self, ContactHelloError>` → return `ContactHelloError` with out parameter for success data
   - `Result<bool, ContactHelloError>` → return `ContactHelloError` with `bool*` out parameter

### 3. **Option Types**
   - **Rust**: `Option<[u8; 32]>` for optional avatar hash
   - **C**: Used `NULL` pointer to indicate `None`, with explicit checks: `if (avatar_hash != NULL)`

### 4. **Struct Methods**
   - **Rust**: Methods defined in `impl` blocks
   - **C**: Converted to functions taking struct pointers as first parameter
   - Example: `hello.sign(&key)` → `contact_hello_sign(&hello, key)`

### 5. **Enum Representation**
   - **Rust**: `#[derive(Debug, Clone, Copy, PartialEq, Eq)]` enums
   - **C**: Standard C enums with explicit values where specified
   - `ContactHelloError` and `TrustLevel` translated directly

### 6. **HashMap Implementation**
   - **Rust**: Uses `heapless::FnvIndexMap` for the contact store
   - **C**: Implemented using a simple array-based hash map with linear probing
   - Used FNV-1a hash algorithm for key hashing (matching heapless behavior)
   - Includes occupied flag for tracking used slots

### 7. **Byte Order Conversion**
   - **Rust**: Uses `.to_le_bytes()` and `from_le_bytes()` for endianness
   - **C**: Manually implemented little-endian encoding/decoding with bitwise operations
   - All multi-byte values (u16, u32, u64) explicitly converted to ensure portability

### 8. **Conditional Compilation**
   - **Rust**: `#[cfg(target_arch = "xtensa")]` for ESP-IDF specific code
   - **C**: Used `#ifdef __XTENSA__` with stub implementations for non-xtensa platforms
   - ESP-IDF NVS functions called directly with proper error handling

### 9. **String/Byte Array Handling**
   - **Rust**: Uses `b"string"` syntax for byte string literals
   - **C**: Used character arrays with explicit initialization: `{'Y', 'C', 'H', ':'}`
   - C-string compatibility maintained for NVS key names with null termination

### 10. **Memory Management**
   - **Rust**: Stack allocation with heapless vectors
   - **C**: All structures use stack allocation (no dynamic memory)
   - Fixed-size arrays ensure no heap usage, matching embedded constraints

### 11. **Cryptographic Functions**
   - **Rust**: Assumes `Ed25519` and `Sha256` types from crypto module
   - **C**: Declared function prototypes assuming external implementation
   - Signature type converted to struct: `Signature(bytes)` → `Ed25519Signature { uint8_t bytes[64]; }`

### 12. **Iterator Pattern**
   - **Rust**: Provides `iter()` method returning iterator
   - **C**: Not directly translated; users should iterate over `entries` array and check `occupied` flag
   - Comment notes this deviation from original API

### 13. **Constants**
   - All `pub const` declarations converted to `#define` macros
   - Maintains exact same values and naming (uppercase with underscores)

### 14. **Logging**
   - **Rust**: Uses `::log::info!` macro
   - **C**: Commented out but can be replaced with printf or custom logging
   - Preserved log messages as comments for reference

### 15. **Namespace Handling**
   - **Rust**: NVS namespace as byte slice with null terminator: `b"contacts\0"`
   - **C**: Declared as null-terminated C string: `const char NVS_CONTACT_NAMESPACE[] = "contacts";`

### Potential Issues and Limitations:
1. **Cryptographic Dependencies**: The code requires external implementations of `sha256_hash()`, `ed25519_sign()`, and `ed25519_verify()` functions with appropriate signatures
2. **Iterator API**: The `iter()` method is not directly translatable; C users must manually iterate over the entries array
3. **Logging**: Logging statements are commented out; users should integrate with their logging system
4. **Error Propagation**: C's lack of built-in Result types means error handling is more verbose
5. **Type Safety**: C provides less compile-time safety than Rust; users must be careful with pointer validity and buffer sizes
...
// Conditional compilation for ESP-IDF
#ifdef __XTENSA__
...
static ContactHelloError heapless_vec128_extend_from_slice(HeaplessVec128* vec, const uint8_t* data, size_t len) {
    if (vec->len + len > MAX_DID_LENGTH) {
        return CONTACT_HELLO_ERROR_DID_TOO_LONG;
    }
...
static ContactHelloError heapless_vec64_extend_from_slice(HeaplessVec64* vec, const uint8_t* data, size_t len) {
    if (vec->len + len > MAX_NAME_LENGTH) {
        return CONTACT_HELLO_ERROR_NAME_TOO_LONG;
    }
    memcpy(vec->data + vec->len, data, len);
    vec->len += len;
    return CONTACT_HELLO_ERROR_NONE;
...
ContactHelloError contact_hello_new(
    ...
    if (err != CONTACT_HELLO_ERROR_NONE) {
        return err;
    }
...
ContactHelloError contact_hello_decode(ContactHello* hello, const uint8_t* data, size_t data_len) {
    // Minimum size check
    if (data_len < 1 + 8 + 2 + 32 + 32 + 1 + 32 + 64) {
        return CONTACT_HELLO_ERROR_INVALID_FORMAT;
    }
    ...
    if (version != CONTACT_HELLO_VERSION) {
        return CONTACT_HELLO_ERROR_INVALID_VERSION;
    }
    pos += 1;
    ...
    if (did_len > MAX_DID_LENGTH || pos + did_len > data_len) {
        return CONTACT_HELLO_ERROR_DID_TOO_LONG;
    }
    ...
    if (name_len > MAX_NAME_LENGTH || pos + name_len > data_len) {
        return CONTACT_HELLO_ERROR_NAME_TOO_LONG;
    }
...
ContactHelloError contact_hello_from_qr_data(ContactHello* hello, const uint8_t* data, size_t data_len) {
    if (data_len < 4 || memcmp(data, "YCH:", 4) != 0) {
        return CONTACT_HELLO_ERROR_INVALID_FORMAT;
    }
...
bool contact_deserialize(Contact* contact, const uint8_t* data, size_t data_len) {
    ...
    if (pos + hello_len > data_len) {
        return false;
    }

    // Decode hello
    ContactHello hello;
    if (contact_hello_decode(&hello, &data[pos], hello_len) != CONTACT_HELLO_ERROR_NONE) {
        return false;
    }
    pos += hello_len;
...
bool contact_store_remove(ContactStore* store, const uint8_t fingerprint[8], Contact* out_contact) {
    for (size_t i = 0; i < MAX_CONTACTS; i++) {
        if (store->entries[i].occupied && keys_equal(store->entries[i].key, fingerprint)) {
            if (out_contact != NULL) {
                *out_contact = store->entries[i].value;
            }
            store->entries[i].occupied = false;
            store->count--;
            return true;
...
const Contact* contact_store_find_by_pubkey(const ContactStore* store, const uint8_t ed25519_public[32]) {
    for (size_t i = 0; i < MAX_CONTACTS; i++) {
        if (store->entries[i].occupied) {
            if (memcmp(store->entries[i].value.hello.ed25519_public, ed25519_public, 32) == 0) {
                return &store->entries[i].value;
            }
        }
    }
