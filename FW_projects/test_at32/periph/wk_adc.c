...
void wk_adc1_init(void)
{
  ...
  gpio_init_struct.gpio_pins = VBATT_PIN;
  gpio_init(VBATT_GPIO_PORT, &gpio_init_struct);
...
void kick_adc(void)
{
	...
	gpio_bits_set(BATT_MEAS_GPIO_PORT,BATT_MEAS_PIN);
	adc_ordinary_software_trigger_enable(ADC1,TRUE);
}

void adc_irq_process(void)
{
	uint16_t adc_val = adc_ordinary_conversion_data_get(ADC1);
	//calculate voltage
	Vbatt = (((double)V_REF/4096.0) * adc_val * K_VBAT) / 1000.0;
	//turn off measuring circuit
	gpio_bits_reset(BATT_MEAS_GPIO_PORT,BATT_MEAS_PIN);
}
/* add user code end 1 */
