...
{
  if (queue->count >= TX_QUEUE_DEPTH) return false;
...
{
  if (queue->count >= FROM_RADIO_QUEUE_SIZE) return false;
  queue->items[queue->tail] = *item;
...
{
  if (data_len > ble_connection_max_payload(conn)) return BLE_ERROR_MTU_EXCEEDED;
  DataVec vec;
  if (data_len > MAX_MTU) return BLE_ERROR_MTU_EXCEEDED;
  memcpy(vec.data, data, data_len);
  vec.len = data_len;
  if (!tx_queue_push_back(&conn->tx_queue, &vec)) return BLE_ERROR_QUEUE_FULL;
  return BLE_ERROR_OK;
}
...
static void ble_state_init(BleState *state, const char *name) 
{
  ...
  for (size_t i = 0; i < MAX_CONNECTIONS; i++) state->connections[i].valid = false;
  // Copy device name
...
static BleConnection *ble_state_find_connection(BleState *state, uint16_t handle) 
{
  ...
  {
    if (state->connections[i].valid && state->connections[i].handle == handle) return &state->connections[i];
  }
  return NULL;
}
...
{
  if (manager->initialized) return BLE_ERROR_ALREADY_INITIALIZED;
  ...
  if (rc != 0) return BLE_ERROR_STACK_ERROR;
...
{
  if (!manager->initialized) return BLE_ERROR_NOT_INITIALIZED;
#ifdef __XTENSA__
  ...
  int rc = ble_gap_adv_start(BLE_OWN_ADDR_PUBLIC, NULL, BLE_HS_FOREVER,&adv_params, gap_event_callback, NULL);
  if (rc != 0) return BLE_ERROR_STACK_ERROR;
#endif
...
static BleError ble_manager_stop_advertising(BleManager *manager) 
{
  ...
  if (rc != 0 && rc != BLE_HS_EALREADY) return BLE_ERROR_STACK_ERROR;
#endif
...
#ifdef __XTENSA__
static BleError ble_manager_send_notification(const BleManager *manager, uint16_t conn_handle,uint16_t char_handle, const uint8_t *data, size_t data_len) 
{
  os_mbuf *om = ble_hs_mbuf_att_pkt();
  if (om == NULL) return BLE_ERROR_STACK_ERROR;
...
static BleError ble_manager_send(const BleManager *manager, uint16_t conn_handle,const uint8_t *data, size_t data_len) 
{
  if (!manager->initialized) return BLE_ERROR_NOT_INITIALIZED;
  BleError result;
  ...
  if (result != BLE_ERROR_OK) return result;
#endif
  return BLE_ERROR_OK;
}
                                      ...
                                      const uint8_t *data, size_t data_len, size_t *sent_count) 
{
  ...
  for (size_t i = 0; i < MAX_CONNECTIONS; i++) 
  {
    ...
    if (conn->valid && conn->service == service && ble_connection_is_subscribed(conn)) 
    {
      if (data_len <= ble_connection_max_payload(conn)) 
      {
        if (ble_connection_queue_tx(conn, data, data_len) == BLE_ERROR_OK) sent++;
      }
    }
  }
  unlock_state();
  if (sent_count != NULL) *sent_count = sent;
  return BLE_ERROR_OK;
}
...
static BleError ble_manager_notify_from_num(const BleManager *manager, uint32_t counter, size_t *sent_count) 
{
  ...
  for (size_t i = 0; i < MAX_CONNECTIONS; i++) 
  {
    ...
    {
#ifdef __XTENSA__
      if (ble_manager_send_notification(manager, conn->handle, from_num_handle,counter_bytes, 4) == BLE_ERROR_OK) sent++;
#else
      sent++;
#endif
    }
...
static bool ble_manager_read_with_service(const BleManager *manager, BleDataPacket *packet) 
{
  ...
  // Copy back
  while (event_queue_pop_front(&temp, &ev)) event_queue_push_back(&g_event_queue, &ev);
  unlock_event();
  // Fill output packet
