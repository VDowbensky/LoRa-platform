...
typedef struct RngHealthStats 
{
  ...
  uint32_t estimated_entropy;// Estimated entropy level (scaled)
} RngHealthStats;
...
static void update_health_state(uint32_t value) 
{
  ...
  else atomic_store_explicit(&REPETITION_COUNT, 0, memory_order_seq_cst);
  // Update bit counts for entropy estimation
  for (int i = 0; i < 32; i++) 
  {
    if (((value >> i) & 1) == 1) atomic_fetch_add_explicit(&BIT_COUNTS[i], 1, memory_order_relaxed);
  }
  // Periodically reassess health
  if (count > 0 && count % APT_WINDOW_SIZE == 0) reassess_health();
}
...
static void reassess_health(void) 
{
  ...
  if (failures == 0 && entropy >= MIN_ENTROPY_SCALED) atomic_store_explicit(&RNG_HEALTHY, true, memory_order_seq_cst);
  ...
  if (failures > 0) 
  {
    ...
    atomic_compare_exchange_strong_explicit(&FAILURE_COUNT,&expected,failures - 1,memory_order_seq_cst,memory_order_relaxed);
  }
  // Periodically reset bit counts
  if (atomic_load_explicit(&SAMPLE_COUNT, memory_order_seq_cst) % (APT_WINDOW_SIZE * 4) == 0) 
  {
    for (int i = 0; i < 32; i++) atomic_store_explicit(&BIT_COUNTS[i], 0, memory_order_relaxed);
  }
}
...
static uint32_t estimate_entropy(void) 
{
  ...
  if (samples < MIN_SAMPLES_FOR_HEALTH) return 0;
  ...
  for (int i = 0; i < 32; i++) 
  {
    ...
    if (diff >= expected) scaled_entropy = 0;
    else 
    {
...
{
  bool expected = false;
  if (!atomic_compare_exchange_strong_explicit(&INITIALIZED,&expected,true,memory_order_seq_cst,memory_order_seq_cst)) return;// Already initialized
  ...
  for (uint32_t i = 0; i < MIN_SAMPLES_FOR_HEALTH; i++) (void)raw_random_u32_with_health();
  uint32_t failures = atomic_load_explicit(&FAILURE_COUNT, memory_order_seq_cst);
  ...
  if (!healthy) log_rng_warning("RNG health check failed during initialization");
}
...
{
  if (!atomic_load_explicit(&INITIALIZED, memory_order_seq_cst)) init();
  ...
  for (int i = 0; i < 5; i++) spin_loop_hint();
  // Second sample
  ...
  if (!healthy) 
  {
    ...
    {
      for (int j = 0; j < 10; j++) spin_loop_hint();
