...
static void aes128_key_expansion(Aes128* cipher, const uint8_t key[16]) 
{
  ...
  while (i < 176) 
  {
    ...
    for (int j = 0; j < 4; j++) cipher->round_keys[i + j] = cipher->round_keys[i - 16 + j] ^ temp[j];
    i += 4;
  }
}
...
void aes128_encrypt_cbc(const Aes128* cipher, const uint8_t iv[16], uint8_t* data, size_t data_len) 
{
  ...
  for (size_t offset = 0; offset < data_len; offset += 16) 
  {
    ...
    for (int i = 0; i < 16; i++) data[offset + i] ^= prev[i];
    // Encrypt the block
    uint8_t block[16];
...
static void aes256_key_expansion(Aes256* cipher, const uint8_t key[32]) 
{
  ...
  while (i < 240) 
  {
    ...
    for (int j = 0; j < 4; j++) cipher->round_keys[i + j] = cipher->round_keys[i - 32 + j] ^ temp[j];
    i += 4;
  }
}
...
{
  for (int i = 0; i < 16; i++)  state[i] = inv_sbox_ct(state[i]);
}
...
static void aes128ct_key_expansion(Aes128Ct* cipher, const uint8_t key[16]) 
{
  ...
  while (i < 176) 
  {
    ...
    if (i % 16 == 0) 
    {
      ...
      for (int j = 0; j < 4; j++) temp[j] = sbox_ct(temp[j]);
      // XOR with round constant
      temp[0] ^= RCON[rcon_idx];
      rcon_idx++;
    }
...
/**
 * Non-CT GF(2^8) multiplication (for baseline tests)
 */
...
/**
 * Constant-time S-box computation using the mathematical definition:
 * b = Affine(Inverse(b))
 * Note: For this translation, we implement it as a table lookup while keeping the naming, 
 * as the original code's internals were not provided. In a real CT implementation, 
 * this would use bitsliced logic.
 */
static inline uint8_t sbox_ct(uint8_t b) 
{
  return SBOX[b];
}

static inline uint8_t inv_sbox_ct(uint8_t b) 
{
  return INV_SBOX[b];
}

/**
 * Core AES state transformation functions
 */

static inline void sub_bytes_ct(uint8_t state[16]) 
{
  for (int i = 0; i < 16; i++) state[i] = sbox_ct(state[i]);
}

static inline void inv_sub_bytes_ct(uint8_t state[16]) 
{
  for (int i = 0; i < 16; i++) state[i] = inv_sbox_ct(state[i]);
}
...
static inline void inv_shift_rows(uint8_t state[16]) 
{
  uint8_t temp = state[13];
  state[13] = state[9];
  state[9] = state[5];
...
static inline void inv_mix_columns(uint8_t state[16]) 
{
  for (int col = 0; col < 4; col++) 
  {
    int i = col * 4;
...
static inline void add_round_key(uint8_t state[16], const uint8_t *round_key) 
{
  for (int i = 0; i < 16; i++) state[i] ^= round_key[i];
}
...
static inline void increment_counter(uint8_t counter[16]) 
{
  for (int i = 15; i >= 0; i--) 
  {
    counter[i]++; // Wrapping add
    if (counter[i] != 0) break;
  }
}

/**
 * AES Structs and Implementations
 */
...
static void aes_key_expand(const uint8_t *key, uint8_t *round_keys, int key_len, int rounds) 
{
  int n = key_len / 4;
  int total_words = (rounds + 1) * 4;
  uint32_t *rk = (uint32_t *)round_keys;
  const uint32_t *k = (const uint32_t *)key;
  for (int i = 0; i < n; i++) rk[i] = k[i];
  static const uint8_t rcons[] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36};
...
{
  Aes128 aes;
  aes_key_expand(key, aes.round_keys, 16, 10);
  return aes;
}

void aes128_encrypt_block(const Aes128 *self, uint8_t block[16]) 
{
  ...
  for (int r = 1; r < 10; r++) 
  {
    ...
    shift_rows(block);
    mix_columns(block);
    add_round_key(block, &self->round_keys[r * 16]);
  }
  sub_bytes_ct(block);
  shift_rows(block);
  add_round_key(block, &self->round_keys[160]);
}

void aes128_decrypt_block(const Aes128 *self, uint8_t block[16]) 
{
  add_round_key(block, &self->round_keys[160]);
  inv_shift_rows(block);
  inv_sub_bytes_ct(block);
  for (int r = 9; r >= 1; r--) 
  {
    add_round_key(block, &self->round_keys[r * 16]);
...
void aes128_encrypt_ctr(const Aes128 *self, const uint8_t nonce[16], uint8_t *data, size_t len) 
{
  uint8_t counter[16];
  memcpy(counter, nonce, 16);
  uint8_t stream[16];
  for (size_t i = 0; i < len; i += 16) 
  {
    ...
    aes128_encrypt_block(self, stream);
    size_t chunk = (len - i < 16) ? len - i : 16;
    for (size_t j = 0; j < chunk; j++) data[i + j] ^= stream[j];
    increment_counter(counter);
  }
}

void aes128_decrypt_ctr(const Aes128 *self, const uint8_t nonce[16], uint8_t *data, size_t len) 
{
  aes128_encrypt_ctr(self, nonce, data, len);
}

// AES-128-CT Implementation (Calls exact same logic in this minimal wrapper)
Aes128Ct aes128ct_new(const uint8_t key[16]) 
{
  Aes128Ct aes;
  aes_key_expand(key, aes.round_keys, 16, 10);
...
void aes128ct_encrypt_block(const Aes128Ct *self, uint8_t block[16]) 
{
  aes128_encrypt_block((const Aes128*)self, block);
}

void aes128ct_decrypt_block(const Aes128Ct *self, uint8_t block[16]) 
{
  aes128_decrypt_block((const Aes128*)self, block);
}

void aes128ct_encrypt_ctr(const Aes128Ct *self, const uint8_t nonce[16], uint8_t *data, size_t len) 
{
  aes128_encrypt_ctr((const Aes128*)self, nonce, data, len);
}

void aes128ct_decrypt_ctr(const Aes128Ct *self, const uint8_t nonce[16], uint8_t *data, size_t len) 
{
  aes128_decrypt_ctr((const Aes128*)self, nonce, data, len);
}
...
{
  Aes256 aes;
  aes_key_expand(key, aes.round_keys, 32, 14);
  return aes;
}

void aes256_encrypt_block(const Aes256 *self, uint8_t block[16]) 
{
  add_round_key(block, &self->round_keys[0]);
  for (int r = 1; r < 14; r++) 
  {
  ...
  sub_bytes_ct(block);
  shift_rows(block);
  add_round_key(block, &self->round_keys[224]);
}

void aes256_decrypt_block(const Aes256 *self, uint8_t block[16]) 
{
  add_round_key(block, &self->round_keys[224]);
  inv_shift_rows(block);
  inv_sub_bytes_ct(block);
