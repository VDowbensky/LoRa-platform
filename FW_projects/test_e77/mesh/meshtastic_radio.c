 ...
 */
 //Must be rewritten according to actual radio configuration.
uint32_t meshSlotTimeMs(MeshModemPreset_t preset) 
{
  ModemParams_t mp = meshPresetParams(preset);
...
uint32_t meshTxDelayMs(MeshModemPreset preset, float channel_util_pct = 0) 
{
  ...
  uint8_t cw_size = MESH_CW_MIN + (uint8_t)((MESH_CW_MAX - MESH_CW_MIN) * (channel_util_pct / 100.0f));
...
uint32_t meshRebroadcastDelayMs(MeshModemPreset preset, MeshRole role, float snr) 
{
  ...
  uint8_t cw_size = MESH_CW_MIN + (uint8_t)((MESH_CW_MAX - MESH_CW_MIN) * ((snr_clamped - (-20.0f)) / 30.0f));
  if (role == ROLE_CLIENT_MUTE) return UINT32_MAX; // never rebroadcast
 ...
 */
MeshPacketIdGen_t  MeshPacketIdGen_t;
uint32_t counter;
...
uint32_t next() 
{
  ...
  counter++;
  if ((counter & 0x3FF) == 0) random_base = (uint32_t)rand() & 0xFFFFFC00u; // Wrap: pick new random base
  return id;
}
...
MeshRxResult_t MeshRxResult;
MeshSession_t  MeshSession;
MeshRegion_t        region;
...
{
  memset(result,0,sizeof(MeshRxResult_t));
  // Parse header
  ...
  // Conditions: channel_hash==0, unicast to us, sender key known, payload > 12 bytes overhead
  if (pki.initialized && meshIsPkiCandidate(&result->packet, node_num)) 
  {
    uint16_t pt_len = meshTryPkiDecrypt(&result->packet, &pki,&node_keys, plaintext);
    if (pt_len > 0) 
    {
      // Validate protobuf decode
      if (meshDecodeData(plaintext, pt_len, &result->data) && result->data.portnum != PORT_UNKNOWN)
      {
        decrypted = true;
        result->is_pki = true;
  ...
  if (!decrypted) 
  {
    ...
    int ch_idx = channels.tryDecrypt(result->packet.payload, result->packet.payload_len,
                                    result->packet.channel_hash,result->packet.from, result->packet.id,
                                    plaintext, &matched_key,meshValidateData);
    if (ch_idx >= 0) 
    {
                            ...
                            bool want_ack,bool ok_to_mqtt,uint8_t *out,uint8_t tx_hop_limit)
{
  ...
  uint16_t data_len = meshEncodeData(data_buf,sizeof(data_buf),portnum,payload,payload_len,false,ok_to_mqtt);
  ...
  if (ch->key.length > 0) 
  {
    if (!meshEncrypt(&ch->key,node_num,pkt_id,data_buf,data_len)) return 0;
  }
  ...
  return meshBuildPacket(out,to,node_num, pkt_id,hop_limit, hop_start,want_ack,false,  // via_mqtt=false always
                          ch->hash,0,0,  // next_hop, relay_node
                          data_buf, data_len);
}

//Build a PKI-encrypted DM frame (direct message to specific node).
//Uses x25519 DH + AES-256-CCM. Requires recipient's public key in node_keys.
//Returns total frame length, or 0 on failure.
                              ...
                              bool want_ack,uint8_t *out,uint32_t (*rand_fn)(void),uint8_t tx_hop_limit)
{
  ...
  uint8_t data_buf[240];
  uint16_t data_len = meshEncodeData(data_buf,sizeof(data_buf),portnum,payload,payload_len);
  if (data_len == 0) return 0;
  // PKI encrypt (produces ciphertext + 12 bytes overhead)
  uint8_t encrypted[256];
  uint32_t pkt_id = id_gen.next();
  uint16_t enc_len = meshPkiEncrypt(pki.private_key, their_key,node_num, pkt_id,data_buf,data_len,encrypted,rand_fn);
  if (enc_len == 0) return 0;
  // PKI DMs use channel_hash=0 (Router.cpp:453)
  uint8_t hop_limit = tx_hop_limit & 0x07;
  uint8_t hop_start = hop_limit;
  return meshBuildPacket(out,to_node, node_num, pkt_id,hop_limit, hop_start,want_ack,false,  // via_mqtt=false always
                          0,  // channel_hash = 0 for PKI DMs
                          0, 0,encrypted,enc_len);
}
...
{
  return MeshSession_buildDmTx(to_node, PORT_TEXT_MESSAGE,(const uint8_t *)text,strlen(text),want_ack,out,rand_fn,tx_hop_limit);
}
...
{
  return MeshSession_buildTx(channel_idx,to,PORT_TEXT_MESSAGE,(const uint8_t *)text,strlen(text),want_ack,ok_to_mqtt,out,tx_hop_limit);
}
                                    ...
                                    uint8_t public_key_len,uint8_t tx_hop_limit)
{
  ...
  uint16_t user_len = meshEncodeUser(user_buf,sizeof(user_buf),id,long_name,short_name,hw_model,public_key,public_key_len);
  if (user_len == 0) return 0;
  return MeshSession_buildTx(channel_idx,to,PORT_NODEINFO,user_buf,user_len,want_ack,ok_to_mqtt,out,tx_hop_limit);
}
...
/* MeshFreqConfig_t meshCalcFrequency(const RegionDef_t *region,MeshModemPreset_t preset, const char *channel_name,int32_t slot_override = -1)
{
  ...
  float bw = mp.bw_khz;
  uint32_t numChannels = (uint32_t)floor((region->freq_end - region->freq_start) / (bw / 1000.0f));
  if (numChannels == 0) numChannels = 1;
  uint32_t channel_num;
  if (slot_override >= 0 && (uint32_t)slot_override < numChannels) channel_num = (uint32_t)slot_override;
  else channel_num = meshDjb2Hash(channel_name) % numChannels;
                                                     ...
                                                     int32_t slot_override = -1)
{
  ...
  if (power == 0 || power > (int8_t)rd->power_limit) power = rd->power_limit;
...
MeshRadioConfig_t radioConfig(int8_t tx_power)
{
