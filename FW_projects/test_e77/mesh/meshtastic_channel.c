...
uint8_t MeshChannel_meshXorHash(const uint8_t *p, uint16_t len) 
{
  uint8_t code = 0;
  for (uint16_t i = 0; i < len; i++) code ^= p[i];
  return code;
}
...
uint8_t       count;
MeshModemPreset_t preset;  // needed for default channel name resolution
 ...
 * Signature: bool validate(const uint8_t *plaintext, uint16_t len)
 */
int MeshChannelTable_tryDecrypt(const uint8_t *payload, uint16_t payload_len,
               uint8_t wire_hash,
