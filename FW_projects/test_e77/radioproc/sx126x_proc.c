...
sx126x_status_t SX126x_init(void)
{
	...
	//if(err != RADIO_OK) return err;
	err = (int8_t)sx126x_set_dio_irq_params(NULL,SX126X_IRQ_TX_DONE | SX126X_IRQ_HEADER_ERROR | SX126X_IRQ_RX_DONE | SX126X_IRQ_CRC_ERROR,SX126X_IRQ_TX_DONE | SX126X_IRQ_HEADER_ERROR | SX126X_IRQ_RX_DONE,SX126X_IRQ_NONE,SX126X_IRQ_NONE);
	if(err != RADIO_OK) return err;
...
int8_t SX126X_setopmode(uint8_t mode)
{
  switch(mode)
  {
    ...
    opmode = RADIO_OPMODE_RX;
		//sx126x_set_dio_irq_params(NULL,SX126X_IRQ_RX_DONE | SX126X_IRQ_CRC_ERROR,SX126X_IRQ_RX_DONE,SX126X_IRQ_NONE,SX126X_IRQ_NONE);
		SX126X_rfsw_rx();
...
void SX126X_irq_handler(void)
{
	...
	if(irqstatus & SX126X_IRQ_HEADER_ERROR) header_error = true;
