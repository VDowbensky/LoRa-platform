 ...
 */
void hkdf_expand(const uint8_t prk[SHA256_DIGEST_SIZE],const uint8_t *info, size_t info_len,uint8_t *okm, size_t okm_len) 
{
  ...
  for (size_t i = 1; i <= n; i++) 
  {
    ...
    if (i > 1) hmac_sha256_update(&hmac, t, SHA256_DIGEST_SIZE);
    ...
    size_t to_copy = (SHA256_DIGEST_SIZE < (okm_len - offset)) ? SHA256_DIGEST_SIZE : (okm_len - offset);
 ...
 * @param okm_len Length of OKM to generate
 */
void hkdf_derive(const uint8_t *salt, size_t salt_len,const uint8_t *ikm, size_t ikm_len,
 ...
 */
void reticulum_identity_hash(const uint8_t signing_key[32],const uint8_t encryption_key[32],uint8_t hash[RETICULUM_IDENTITY_HASH_SIZE]) 
{
  sha256_ctx hasher;
 ...
 */
void reticulum_full_identity_hash(const uint8_t signing_key[32],const uint8_t encryption_key[32],uint8_t hash[RETICULUM_FULL_HASH_SIZE]) 
{
  sha256_ctx hasher;
  sha256_init(&hasher);
  sha256_update(&hasher, signing_key, 32);
 ...
 */
void reticulum_derive_link_keys(const uint8_t shared_secret[32],const uint8_t initiator_pub[32],const uint8_t responder_pub[32],reticulum_link_keys *keys) 
{
  ...
  hkdf_derive(salt, sizeof(salt) - 1,shared_secret, 32,context, 64,master, 64);// Don't include null terminator
 ...
 */
void meshcore_derive_channel_key(const uint8_t *psk, size_t psk_len,uint8_t channel_id,uint8_t key[32]) 
{
  const uint8_t salt[] = "meshcore";
  uint8_t info[3] = {'C', 'H', channel_id};
  hkdf_derive(salt, sizeof(salt) - 1,psk, psk_len,info, 3,key, 32); // Don't include null terminator
}
 ...
 * @param key Output key (32 bytes)
 */
void meshcore_derive_node_key(const uint8_t identity[32],const uint8_t *purpose, size_t purpose_len,uint8_t key[32]) 
{
  const uint8_t salt[] = "meshcore-node";
  hkdf_derive(salt, sizeof(salt) - 1,identity, 32,purpose, purpose_len,key, 32); // Don't include null terminator
}
 ...
 * @param key Output key (32 bytes)
 */
void meshtastic_derive_channel_key(const char *channel_name,size_t channel_name_len,uint8_t key[32]) 
{
 ...
 */
void meshtastic_derive_nonce(uint32_t packet_id, uint32_t sender,uint8_t nonce[16]) 
{
  memset(nonce, 0, 16);
