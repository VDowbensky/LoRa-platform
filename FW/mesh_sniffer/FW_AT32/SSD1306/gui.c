...
//////////////////////////////////////////////////////////////////////////////////	 
//This program is for learning purposes only. It may not be used for any other purpose without the author's permission.
//Test hardware: MCU STM32F103RCT6, Zhengdian Atom MiniSTM32 development board, main frequency 72MHZ, crystal oscillator 12MHZ
//QDtech-OLED LCD driver for STM32
...
//Our company provides technical support. Welcome to communicate and learn any technical problems at any time
//Landline (Fax): +86 0755-23594567
...
//Version: V1.0
//All rights reserved. Piracy will be investigated.
...
{  	
	uint8_t i,j;			
	uint8_t width=ex-sx+1; 		//Get the padding width
	uint8_t height=ey-sy+1;		//high
	for(i=0;i<height;i++)
	{
		for(j=0;j<width;j++)
		{
				SSD1306_Set_Pixel(sx+j, sy+i,color);
		}		
	}
	SSD1306_Display();
}
...
void GUI_DrawLine(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2,uint8_t color)
{
	...
	else if(delta_x==0)incx=0;//Vertical Lines
	...
	{ 
		SSD1306_Set_Pixel(uRow,uCol,color);
		xerr+=delta_x ; 
		yerr+=delta_y ; 
		if(xerr>distance) 
		{ 
...
{
	SSD1306_Set_Pixel(xc + x, yc + y, color);
	SSD1306_Set_Pixel(xc - x, yc + y, color);
	SSD1306_Set_Pixel(xc + x, yc - y, color);
	SSD1306_Set_Pixel(xc - x, yc - y, color);
	SSD1306_Set_Pixel(xc + y, yc + x, color);
	SSD1306_Set_Pixel(xc - y, yc + x, color);
	SSD1306_Set_Pixel(xc + y, yc - x, color);
	SSD1306_Set_Pixel(xc - y, yc - x, color);
}
			...
			for(i=0;i<16;i++)
			{
			  if(mode)
				...
				for(j=0;j<8;j++)
				{
					if(tmp&(0x80>>j))
					{
						SSD1306_Set_Pixel(x+j, y+i,1);
					}
					else
					{
						SSD1306_Set_Pixel(x+j, y+i,0);
					}
				}
			}
		}
		...
		for(j=0;j<(width+7)/8;j++)
		{
		    if(mode)
				...
				for(k=0;k<8;k++)
				{
					if(tmp&(0x80>>k))
					{
						SSD1306_Set_Pixel(x+j*8+k, y+i,1);
					}
					else
					{
						SSD1306_Set_Pixel(x+j*8+k, y+i,0);
					}
				}
		}
 } 
